...
*/
// Starting off with defining the necessary header files
...
//Defining the the function named clientFunction that takes in arguments namely, socket file descriptor and the arguments that are passed from the user to the terminal.( argv = array of pointers).
void clientFunction(int socket_fd, char *argv[]) {
	...
	int fdpipe[2]; // Defining file descriptor for piping.
	if(pipe(fdpipe) < 0) {
		// Checking if the pipe is initialized with fd >0 otherwise , it will show the corresponding error.
...
int main(int argc, char *argv[])
{
	...
	int socket_fd, connection_fd; // Initializing socket and the connection file descriptors.
